...
def get_order_summary(period: str = "today") -> str:
    ...
    rows = _run_query("""
        SELECT
            ...
            COALESCE(SUM(CASE WHEN status = 'complete' THEN amount ELSE 0 END), 0) AS revenue,
...
def get_package_performance(period: str = "this_month") -> str:
    ...
    rows = _run_query("""
        SELECT
            ...
            COALESCE(SUM(CASE WHEN o.status = 'complete' THEN o.amount ELSE 0 END), 0) AS revenue
        FROM orders o
...
def get_ai_sales_suggestions() -> str:
    ...
    month = _run_query("""
        SELECT
            ...
            COALESCE(SUM(CASE WHEN status='complete' THEN amount END), 0) revenue
    ...
    top_packages = _run_query("""
        SELECT p.name, c.name cat, COUNT(*) cnt,
               COALESCE(SUM(CASE WHEN o.status='complete' THEN o.amount END), 0) rev,
